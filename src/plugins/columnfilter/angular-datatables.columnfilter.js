    ...
    function initColumnFilterPlugin(DTRendererService) {
        ...
        function postRender(options, result) {
            if (options && options.hasColumnFilter) {
                if (!result.dataTable.columnFilter){
                    result.dataTable.columnFilter = columnFilter
                }
...
var columnFilter = function (options) {
    ...
    return this.each(function () {
        ...
        if (properties.sPlaceHolder == "head:after") {
          ...
          //   tr.children().filter(function(x, y){
          ...
          //  tr.appendTo($(oTable.fnSettings().nTHead));
            if (oTable.fnSettings().bSortCellsTop) {
                ...
                myTR.appendTo($(oTable.fnSettings().nTHead));
                var head = [];
                for(x of oTable.fnSettings().nTHead.rows[1].cells){
                  x.classList.remove('sorting');
                  x.innerText = '';
                  head.push({cell:x})
                }
                oTable.fnSettings().aoHeader.push(head);
